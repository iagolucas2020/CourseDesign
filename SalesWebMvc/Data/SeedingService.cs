...
{
    public class SeedingService // SERVICE FOR POPULAR DATE BASE
    {
        private SalesWebMvcContext _context;

        public SeedingService(SalesWebMvcContext context) // CONSTRUTOR
        {
            _context = context;
        }

        public void Seed() // RESPONSIBLE FOR POPULAR DATE BASE
        {
            if (_context.Department.Any() ||
                _context.Seller.Any() ||
                _context.SalesRecords.Any())
            {
                return; // DATE BASE WAS SEEDED
            }
